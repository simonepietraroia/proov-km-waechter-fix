...
def car_wear(car: dict) -> float:
    ...
    Returns 0.0 for any car with no 'last_service_km' reading (unknown history).
    """
...
def fleet_summary(fleet: list) -> dict:
    """Return a summary dict with count, number due, and average wear across the fleet."""
    total = 0.0
    due = 0
    for car in fleet:
        total += car_wear(car)
...
def print_report(fleet: list) -> None:
    """Print the nightly fleet-health report and flush it to the configured log file."""
